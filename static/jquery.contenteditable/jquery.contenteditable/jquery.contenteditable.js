...
(function ($) {
    ...
    // helper function that builds the toolbar
        toolbar = function (options) {
            var bar = $('<div class="fresheditor-toolbar btn-toolbar" />');
            $.each(options.enabledCommands, function (groupName, group) {
                var groupEl = $('<div class="btn-group" />').addClass(groupName);
                $.each(group, function (v, command) {
                    groupEl.append(
                        $('<button type="button" class="btn btn-default glyphicon" />')
                                .addClass('toolbar_' + command)
                                .addClass(options.commands[command].glyphicon ? 'glyphicon-' + options.commands[command].glyphicon : "")
                                .attr({ title: options.i18n[command] + " (" + options.commands[command].shortcut + ")", href: "#" })
                                .html((options.commands[command].toolbarHtml && !options.commands[command].glyphicon) ? options.commands[command].toolbarHtml : "")
                        );
                });
                bar.append(groupEl);
            });
...
//		};
            ...
            $.each(options.commands, function (command, opts) {
                ...
                methods[command] = (function (command, opts) {
                    return function () {
                        ...
                        if ($toolbar.find('button.toolbar_' + command).size() == 0) {
                            return false;
                        }
                ...
                var scopedThis = function () {
                    return methods[command].apply(that);
                };

                $toolbar.find('button.toolbar_' + command).click(scopedThis);
    ...
    $.fn.fresheditor.defaults = {
        ...
        commands: {
            ...
            italic: { shortcut: "Ctrl+i", execCommand: "italic", toolbarHtml: "I", glyphicon: "italic" },
            ...
            ol: { shortcut: "Ctrl+Alt+o", execCommand: "insertOrderedList", glyphicon: "list-alt" },
            ul: { shortcut: "Ctrl+Alt+u", execCommand: "insertUnorderedList", glyphicon: "list" },
