...
from ss.organisation.models import Minutes, PrintProgramme
...
from ss.organisation.models import Approval, ApprovalSet
...
class ModelBaseHandler(BaseHandler):
    def has_model_form(self):
        return (hasattr(self, 'model_form') and self.model_form is not None) or hasattr(self, 'queryset')

    def create(self, request, *args, **kwargs):
        if not self.has_model() or not self.has_model_form():
        ...
        if form.is_valid():
            form.save()
            resp = rc.ALL_OK
            message = 'Successfully saved "%s"' % form.cleaned_data['title']
            messages.success(request, message, extra_tags='editing')
            resp.content = {
                'status': 'success',
                'message': message,
            ...
            resp = rc.BAD_REQUEST
            message = 'Form validation failed when creating' + str(errs)
            resp.content = {
                'status': 'error',
                'message': message,
            }
    ...
    def update(self, request, *args, **kwargs):
        ...
        if pkfield not in kwargs:
            resp = rc.BAD_REQUEST
            message = 'Bad Request - pkfield not found in request data'
            resp.content = {
                'status': 'error',
                'message': message,
            }
            return resp
        try:
            item = self.queryset(request).get(pk=kwargs.get(pkfield))
        except ObjectDoesNotExist:
            resp = rc.NOT_FOUND
            message = 'ObjectDoesNotExist for pk %s' % kwargs.get(pkfield)
            resp.content = {
                'status': 'error',
                'message': message,
            }
            ...
            resp = rc.BAD_REQUEST
            message = 'MultipleObjectsReturned for pk %s' % kwargs.get(pkfield)
            resp.content = {
                'status': 'error',
                'message': message,
            }
            return resp
        form = self.model_form(request.PUT, request.FILES, instance=item)
        if form.is_valid():
            form.save()
            resp = rc.ALL_OK
            message = 'Successfully saved "%s"' % form.cleaned_data['title']
            ...
            errs = dict((key, [unicode(v) for v in values]) for key, values in form.errors.items())
            resp = rc.BAD_REQUEST
            message = 'Form validation failed when updating' + str(errs)
            resp.content = {
                'status': 'error',
                'message': message,
            }
    ...
    def delete(self, request, *args, **kwargs):
        ...
        try:
            ...
            inst.delete()
            resp = rc.ALL_OK
            message = 'Successfully deleted "%s"' % inst.title
            messages.success(request, message, extra_tags='editing')
            resp.content = {
                'status': 'success',
                'message': message,
...
class PageHandler(ModelBaseHandler):
    exclude = ()
    model = Page
    model_form = PageForm


class AnonymousSeasonHandler(AnonymousBaseHandler):
    model = Season
    allowed_methods = ('GET',)
    fields = ('title',)


class SeasonHandler(ModelBaseHandler):
    anonymous = AnonymousSeasonHandler
    exclude = ()
    model = Season
    model_form = SeasonAdminForm


class AnonymousFilmHandler(AnonymousBaseHandler):
    model = Film
    allowed_methods = ('GET',)
    fields = ('title',)


class FilmHandler(ModelBaseHandler):
    anonymous = AnonymousFilmHandler
    exclude = ()
    model = Film
    model_form = FilmAdminForm


class AnonymousGigHandler(AnonymousBaseHandler):
    model = Gig
    allowed_methods = ('GET', )
    fields = ('title',)


class GigHandler(ModelBaseHandler):
    anonymous = AnonymousGigHandler
    exclude = ()
    model = Gig
    model_form = GigAdminForm
...
class EventHandler(ModelBaseHandler):
    anonymous = AnonymousEventHandler
    exclude = ()
    model = Event
    model_form = EventAdminForm


class AnonymousFestivalHandler(AnonymousBaseHandler):
    model = Festival
    allowed_methods = ('GET',)
    fields = ('title',)


class FestivalHandler(ModelBaseHandler):
    anonymous = AnonymousFestivalHandler
    exclude = ()
    model = Festival
    model_form = FestivalAdminForm


class AnonymousMeetingHandler(AnonymousBaseHandler):
    model = Meeting
    allowed_methods = ('GET',)
    fields = ('title',)


class MeetingHandler(ModelBaseHandler):
    anonymous = AnonymousMeetingHandler
    exclude = ()
    model = Meeting
    model_form = MeetingAdminForm


class AnonymousMinutesHandler(AnonymousBaseHandler):
    model = Minutes
    allowed_methods = ('GET',)
    fields = ('title',)


class MinutesHandler(ModelBaseHandler):
    anonymous = AnonymousMinutesHandler
    exclude = ()
    model = Minutes
    model_form = MinutesForm


class AnonymousDocumentHandler(AnonymousBaseHandler):
    model = Document
    allowed_methods = ('GET',)
    fields = ('title',)


class DocumentHandler(ModelBaseHandler):
    anonymous = AnonymousDocumentHandler
    exclude = ()
    model = Document
    model_form = DocumentForm
