...
class Migration(DataMigration):
    def forwards(self, orm):
        for f in FILM_FORMATS:
            ff = orm.FilmFormat(name=f[0])
            ff.save()
        for f in orm.Film.objects.all():
            f.filmFormat = orm.FilmFormat.objects.get(name=f.oldFilmFormat)
            f.save()

    def backwards(self, orm):
        for f in orm.Film.objects.all():
            f.oldFilmFormat = f.filmFormat.name
            f.save()
