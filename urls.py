...
urlpatterns = patterns('',
    ...
    (r'^media/(?P<path>.*)$', 'django.views.static.serve', {'document_root': os.path.join(settings.PROJECT_ROOT, 'static')}),
)
urlpatterns += patterns('ss.programming.views',
    ...
    url(r'^on/(?P<year>\d{4})(/w(?P<week>\d{1,2})|/(?P<month>\d{1,2})(/(?P<day>\d{1,2})|)|/?)/?$', 'listPeriod', name='listPeriod'),
    ...
    (r'^on/(?P<year>\d{4})(/w(?P<week>\d{1,2})|/(?P<month>\d{1,2})(/(?P<day>\d{1,2})|)|/?)/feed/?$', listPeriodFeed()),
...
urlpatterns += patterns('ss.organisation.views',
    
    url(r'^org/on/(?P<year>\d{4})/(?P<month>\d{1,2})/text/?$', 'monthReportText', name='month-report-text'),
    ...
    url(r'^org/on/?$', 'reportIndex'),

    url(r'^org/returns/(?P<year>\d{4})/(?P<month>\d{1,2})/?$', 'returnReport', name='return-report'),
    ...
    url(r'^org/returns/?$', 'returnReportIndex'),

    url(r'^org/changelog/?$', 'changeLog'),
...
urlpatterns += patterns('ss.content.views',
